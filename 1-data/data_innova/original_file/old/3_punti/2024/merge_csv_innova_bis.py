...
def fuse_triplets(df, time_col, max_interval_min):
    ...
    while i < len(df):
        ...
        if intervals.max() <= max_interval_min:
            # Fusionner
            fused_numeric = block[numeric_cols].mean()  # moyenne
            ...
            if "datetime" in block.columns:
                dt_mean = block["datetime"].astype("int64").mean()  # moyenne en ns depuis 1970
...
def merge_csv_chanel_bis(SL_list):
    all_SL_dfs = []
    for SL in SL_list:
        csv_path = f"Channel_{SL}_Measurement_Data.csv"
