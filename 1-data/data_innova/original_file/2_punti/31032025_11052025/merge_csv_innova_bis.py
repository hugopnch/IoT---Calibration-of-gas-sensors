...
def fuse_triplets(df, time_col, max_interval_min):
    ...
    while i < len(df):
        ...
        if intervals.max() <= max_interval_min:
            ...
            #         fused_non_numeric[col] = counts.idxmax()  # journée majoritaire
            #     else:
            #         fused_non_numeric[col] = block[col].iloc[0]
            # Colonnes non numériques : on calcule la moyenne temporelle
            fused_non_numeric = {}
            ...
            # non_numerics_col_2 = [c for c in non_numeric_cols if c not in ["datetime","date","hour"]]
            # print("non numeric cols 2",non_numerics_col_2)
            # for col in non_numerics_col_2:          
            #     fused_non_numeric[col] = block[col].mean()
    ...
    # Renommer colonne "Date Time" en "Date & Time"
        ...
        if "datetime" in df.columns:
            if "ORA" in df.columns:
                ...
                # Extraire la date
                date_part = df["datetime"].dt.date
                # Convertir "ORA" en heure (format HH:MM:SS)
                df["ORA"] = pd.to_datetime(df["ORA"], format="%H:%M:%S", errors='coerce').dt.time
                # Reconstruire "datetime" complet
                df["datetime"] = pd.to_datetime(date_part.astype(str) + " " + df["ORA"].astype(str), errors='coerce')
                df["date"]= date_part
                # Renommer "ORA" en "hour" pour la suite
                df.rename(columns={"ORA": "hour"}, inplace=True)
                # print("ORA")
                # df.rename(columns={"ORA": "hour"},inplace=True)
                # print(df["hour"])
                ...
                # print(df["date"])
                # df["datetime"] = (df["date"] +" "+ df["hour"]).dt.strftime("%Y-%m-%d %H:%M:%S")
                # print(df["datetime"])
            else:
                df["datetime"] = pd.to_datetime(df["datetime"], errors='coerce', dayfirst = True)
                df["date"] = df["datetime"].dt.date
        ...
        if "A: Carbon dioxide\n[ppm]" in df.columns or "A: Carbon dioxide(ppm)":
        ...
        if "P : Pressure (mbar)" in df.columns or "P: Pressure\n[mbar]":
            df.rename(columns={"P : Pressure (mbar)": "Pressure"}, inplace=True)
            df.rename(columns={"P: Pressure\n[mbar]": "Pressure"}, inplace=True)
        ...
        useful_cols =[c for c in df.columns if c in ["datetime", "date","hour","CO2","NH3","CH4","Pressure","id_channel"]]
        df = df[useful_cols]
        
        # -------------------------------
        # Sauvegarde CSV individuelle
        output_csv = f"Channel_{SL}_Measurement_Data_formatted_P.csv"
...
def main_merge_csv_innova(date,SL):
    # -------------------------------
    # Monitor Setup
    # -------------------------------
    pdf_path_first = f"{date}_SL{SL_list[0]}.pdf"   #f"data/{date}/{date}_SL{SL_list[0]}.pdf"
    ...
    for SL in SL_list:
        ...
        # Renommer colonne "Date Time" en "Date & Time"
        if "Date Time" or "Date & time" in df_final.columns:
            df_final.rename(columns={"Date Time": "datetime"}, inplace=True)
            
        # Ajouter colonnes Date et Heure
        if "datetime" in df_final.columns:
            df_final["datetime"] = pd.to_datetime(df_final["datetime"], format="%Y-%m-%d %H:%M:%S", errors='coerce')
            df_final["date"] = df_final["datetime"].dt.date
            df_final["hour"] = df_final["datetime"].dt.time
        
        if "A: Carbon dioxide\n[ppm]" in df_final.columns:
            df_final.rename(columns={"A: Carbon dioxide\n[ppm]": "CO2"}, inplace=True)
        if "C: Ammonia\n[ppm]" in df_final.columns:
            df_final.rename(columns={"C: Ammonia\n[ppm]": "NH3"}, inplace=True)
        if "D: Methane\n[ppm]" in df_final.columns:
            df_final.rename(columns={"D: Methane\n[ppm]": "CH4"}, inplace=True)
        
        df_final["id_channel"]= SL
        # Conversion explicite des colonnes pseudo-numériques
        
        columns_str = [c for c in df_final.columns if c not in ["datetime", "date", "hour"]]
        print("column str", columns_str)
        for c in columns_str:
            df_final[c] = pd.to_numeric(df_final[c].astype(str).str.replace(",", ".").str.strip(), errors="coerce")
        # -------------------------------
        # Fusion des triplets (médiane)
        # -------------------------------
        df_final = fuse_triplets(df_final, time_col="datetime", max_interval_min=3)
        df_final["id_channel"] = df_final['id_channel'].astype(int)
        print(f"Completion of fuse_triplets for SL{SL}")
        # Selection of the interesting features
        useful_cols = ["datetime", "date","hour","CO2","NH3","CH4","id_channel"]
        df_final = df_final[useful_cols]
        
        # -------------------------------
        # Sauvegarde CSV individuelle
        output_csv = f"Channel_{SL}_Measurement_Data.csv"
        df_final.to_csv(output_csv, index=False, encoding="utf-8-sig")
        print(f"CSV SL {SL} généré avec fusion des triplets !")
        
        all_SL_dfs.append(df_final)
    ...
    if all_SL_dfs:
        df_merged = pd.concat(all_SL_dfs, ignore_index=True)
        df_merged = df_merged.sort_values(["date", "id_channel", "hour"]).reset_index(drop=True)
        df_merged.to_csv("merged_innova.csv", index=False, encoding="utf-8-sig")
        print("Merge final de toutes les SL généré : merged_innova.csv")
        return df_merged
    else:
        print("Aucun SL à merger")
...
def merge_csv_channel(SL_list):
    all_SL_dfs = []
    for SL in SL_list:
        csv_path = f"firenze/Channel_{SL}_Measurement_Data.csv"
    ...
    if all_SL_dfs:
        df_merged = pd.concat(all_SL_dfs, ignore_index=True)
        if "id_channel" in df_merged:
            df_merged["id_channel"] = df_merged['id_channel'].astype(int) 
        else:
            df_merged["id_channel"] = SL
...
def merge_csv_chanel_bis(SL_list):
    all_SL_dfs = []
    for SL in SL_list:
        csv_path = f"2024/Channel_{SL}_Measurement_Data.csv"
        df = pd.read_csv(csv_path, sep=";", encoding="utf-8",header=1)
        
        # Renommer colonne "Date Time" en "Date & Time"
        if "Date Time" or "Date & time" in df.columns:
            df.rename(columns={"Date Time": "datetime"}, inplace=True)
            df.rename(columns={"Date & Time": "datetime"}, inplace=True)
        ...
        if "A: Carbon dioxide(ppm)" in df.columns:
            df.rename(columns={"A: Carbon dioxide(ppm)": "CO2"}, inplace=True)
        if "C: Ammonia(ppm)" in df.columns:
            df.rename(columns={"C: Ammonia(ppm)": "NH3"}, inplace=True)
        if "D: Methane(ppm)" in df.columns:
            df.rename(columns={"D: Methane(ppm)": "CH4"}, inplace=True)
