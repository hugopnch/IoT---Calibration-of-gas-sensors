...
def read_data(file, debug=False) -> pd.DataFrame:
    ...
    # Vérifier si le dernier séparateur est présent
    needs_fix = not first_line.endswith(sep)
    ...
    if needs_fix:
        ...
        # Créer le dossier "fixed_files" si nécessaire
        fixed_folder = os.path.join(os.path.dirname(file), "fixed_files")
        os.makedirs(fixed_folder, exist_ok=True)

        temp_file = os.path.join(fixed_folder, os.path.basename(file).replace(".csv", "_fixed.csv"))
        with open(temp_file, "w", encoding="utf-8") as f:
            f.writelines(lines)

        df = pd.read_csv(temp_file, sep=sep, thousands=" ")
        if debug:
            print(f"File fixed and saved to: {temp_file}")
    else:
        df = pd.read_csv(file, sep=sep, thousands=" ")
        if debug:
            print("File read successfully, no fix needed.")
...
def extract_df_rabbit_computer(df, legend, debug=False):
    ...
    df_long["datetime"] = pd.to_datetime(df_long["Timestamp"], errors="coerce")
...
def csv_rabbit(type_extract, debug=False):
    ...
    df_all = df_all.dropna(subset=["NH3", "CO2", "H2S", "PM_1_0", "PM_2_5", "PM_4_0", "PM_10", "temperature", "umidity"], how="all")
    ...
    df_grouped.to_csv("merged_rabbit.csv", index=False, encoding="utf-8")
    return df_grouped
