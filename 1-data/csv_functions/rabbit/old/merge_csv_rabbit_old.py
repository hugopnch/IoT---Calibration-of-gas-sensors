...
def extract_df_rabbit_computer(df, legend, debug=False):
    ...
    def convert_date_us_to_fr(date_str):
        try:
            # On essaye d'interpréter la date au format US
            dt = pd.to_datetime(date_str, format='%m/%d/%Y %H:%M')
            # Si ça marche, on renvoie au format français
    ...
    # Supposons que la colonne s'appelle 'date'
    df_long['datetime'] = df['Timestamp'].apply(convert_date_us_to_fr)
...
def csv_rabbit(type_extract, debug=False):
    ...
    df_grouped.to_csv("merged_rabbit.csv", index=False, sep =";", encoding="utf-8")
    return df_grouped
