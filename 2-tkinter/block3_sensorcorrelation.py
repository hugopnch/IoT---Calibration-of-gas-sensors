...
class Block3SCorr(Frame):
    def __init__(self, parent, dataframe_from_bloc_2, type_analysis, flag_sync_timescale, threshold_instrument_dic, flag_threshold_instr, flag_isNA,flag_normalize, callback_on_update=None):
        ...
        self.lignes_sources = []  # Liste des lignes
    ...
    def _build_interface(self):
        ...
        # Bouton pour ajouter une ligne "other"
        Button(self, text="Add Line", command=self.ajouter_ligne).pack(pady=10)

        # Bouton pour valider les groupes
        Button(self, text="Validate Configuration", command=self.validate_groupe).pack(pady=10)

        # Ajouter la ligne de référence par défaut
        self.ajouter_ligne_ref()

        # Ajouter une ligne "other" par défaut
        self.ajouter_ligne_other()

    def ajouter_ligne_ref(self):
    ...
    def ajouter_ligne_other(self):
        print("ajouter_ligne_other")
        row_idx = 2  # Position de la première ligne "other" (ligne 3 dans l'interface)
    ...
    def ajouter_ligne(self):
        ...
        line_num_new_line = row_idx_new_line - 1  # Numéro de la nouvelle ligne (-1 pour enlever ref)
    ...
    def reorganiser_lignes(self):
        ...
        fixed_lines = self.lignes_sources[:2]  # Les deux premières lignes sont fixes
        print("len fixed lines:", len(fixed_lines))
        other_lines = self.lignes_sources[2:]  # Les autres lignes "other" à réorganiser
        print("len other lines:", len(other_lines))
        for idx, ligne in enumerate(other_lines):
            row_idx = idx + 3 # Nouvelle position de la ligne (commence à la ligne 4)
            print("idx:",idx)
    ...
    def get_filter_data(self):
        ...
        ref_line = self.lignes_sources[0]
        ...
        for idx, ligne in enumerate(self.lignes_sources[1:], start=1):
    ...
    def update_from_block2(self, dataframes, type_analysis, flag_sync_timescale):
        ...
        for ligne in self.lignes_sources:
            self.update_id_options(ligne)
