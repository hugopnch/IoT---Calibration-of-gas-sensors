# -*- coding: utf-8 -*-
"""
Created on Tue Oct 21 11:27:59 2025
...
def cluster_datetimes_and_pivot(input_path, output_path,
    ...
    # Pivot avec une colonne par mesure et une colonne id_meteo_station
    df_wide = df.pivot_table(
        index=["datetime", "date", "hour", "location"],
        columns="measure",
        values="value",
        aggfunc=pivot_agg
    ).reset_index()
    df_wide.rename(columns={"location": "id_meteo_station"}, inplace=True)
...
def main(folder_raw, folder_output):
    ...
    for file in csv_files:
        try:
            cluster_datetimes_and_pivot(file, file.replace("raw", "formated"), threshold_seconds=5, rep_method="mean", pivot_agg="first")
